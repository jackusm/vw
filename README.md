...
| `LITESTREAM_RETENTION_CHECK_INTERVAL` | `1h`          | The interval at which retention should be applied. On Fly.io, set this low (e.g. `1m`) — machines auto-stop within minutes, so the default would never fire and generations accumulate in the bucket forever.                    |
